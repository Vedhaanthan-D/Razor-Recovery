...
// Checks the strategy → recovery_attempts row map. Every classified payment must produce exactly
// one attempt, always starting at `pending`, and an unknown strategy must still yield a row (so the
// funnel can't silently lose a payment).
const assert = require("assert");
const { initialAttempt } = require("./recoveryService");
